...
**状態（2026-05-27 / 0.9.0 ベータ時点）**：未実装。塾長判断で v1.1 以降へ繰り越し。

理由：
1. Yom-easy のユーザーは「読みたい文章を貼り付ける」のが主動作。小説・記事・歌詞・課題などにキーワードが含まれる頻度が高く、本文スキャンによる誤検知が読書体験を阻害する
2. 本文の内容をスキャンするのは、プライバシー第一の設計思想と整合しない
3. TTS 自然完了時の通知（「お疲れさま。ここまで読み終わりましたね。」）で、強み肯定の役割は最低限担保されている
4. 実装するなら、本文スキャンではなく **Help/About ページに「支えになる連絡先」を常設する形式** が、4 原則（中立性／個人差の尊重）と整合的。これは当事者の声を聞いてから v1.1+ で検討
...
最終更新：2026-05-27（0.9.0 ベータ公開時に「危機対応コンテンツ」を v1.1+ 繰り越しに改訂）
